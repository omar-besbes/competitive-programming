...
pair<bool, int> findPath(vector<int> &between, int root, int destination,
                         pair<vector<int>, pair<int, int>> R[]) {
   R[root - 1].second.second = true;
   if (root == destination) return {true, 0};
         ...
         for (int j = 0; j < N; j++)
            for (int k = j + 1; k < N; k++)
               if (R[j].second.first == R[k].second.first) {
                  ...
                  if (test)
                     score[R[j].second.first] +=
                         findPath(between, j + 1, k + 1, R).second;
               }
         if (score[0] > score[1])
            cout << "A" << '\n';
