...
long long mini(int l, int r, int index = 1, int seg_l = 0, int seg_r = n - 1) {
   ...
   int seg_m = (seg_l + seg_r) / 2;
   return min(mini(l, min(seg_m, r), 2 * index, seg_l, seg_m),
              mini(max(l, seg_m + 1), r, 2 * index + 1, seg_m + 1, seg_r));
}
