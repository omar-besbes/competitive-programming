...
struct Point {
   ...
   bool between(const Point &b, const Point &c) {
      Point bl({min(b.x, c.x), min(b.y, c.y)}),
...
void solve() {
   ...
   if ((p1 - p2) * (p3 - p4) == 0) {
      // collinear lines
      if (p1.cross(p2, p3) == 0) {
         if (p1.between(p3, p4) || p2.between(p3, p4) || p3.between(p1, p2) ||
             p4.between(p1, p2))
            cout << "YES\n";
         else
            cout << "NO\n";
   ...
   // lines intersect
      ...
      // from the POV of one of the lines, the other line is on its right/left
      // without touching it
      if ((s123 > 0 && s124 > 0) || (s123 < 0 && s124 < 0) ||
          (s341 < 0 && s342 < 0) || (s341 > 0 && s342 > 0)) {
         cout << "NO\n";
      } else {
         cout << "YES\n";
      }
