...
bool is_win(pii x) {
   return x.first == 0 || x.second == 0 || x.first == n - 1 ||
          x.second == m - 1;
}
bool is_valid(pii x) {
...
void solve() {
   ...
   for (int i = 0; i < n; i++) {
      cin >> g[i];
      for (int j = 0; j < m; j++) {
         if (g[i][j] == 'M') monsters.insert({i, j});
         if (g[i][j] == 'A') initial_person = {i, j};
      }
   }
   ...
   while (!person.empty() && !yes) {
      for (auto coo : person)
         if (g(coo) == 'A') {
            if (is_win(coo)) yes = true, final_person = coo;
            for (auto [dxy, _] : d) {
               auto newcoo = coo + dxy;
               if (is_valid(newcoo) && g(newcoo) == '.')
                  g(newcoo) = 'A', next_person.insert(newcoo),
                  pred(newcoo) = coo;
            }
         }
   ...
   if (yes) {
      ...
      for (pii cur = final_person; cur != initial_person; cur = pred(cur))
         for (auto [dxy, direction] : d)
            if (dxy == cur - pred(cur)) path += direction;
