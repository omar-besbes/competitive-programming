...
void assemble(int *d, pair<long long, long long> &BSD,
              pair<long long, long long> &MD, int N, long long number,
              bool *visited) {
   bool construit = true;
   for (int i = 0; i < N; ++i) {
      if (!visited[i]) {
...
int main() {
   ...
   pair<long long, long long> BSD = {number, divisor(number).first},
                              MD = {number, divisor(number).second};
