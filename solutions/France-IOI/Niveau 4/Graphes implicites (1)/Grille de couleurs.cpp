...
bool explore(string *G, bool **visited, pair<int, int> posN,
             pair<int, int> posB) {
   if (posB.first < 0 || posB.first >= H || posB.second < 0 || posB.second >= L)
      return false;
   if (posN.first < 0 || posN.first >= H || posN.second < 0 || posN.second >= L)
      return false;
   if (G[posN.first][posN.second] != G[posB.first][posB.second]) return false;
   ...
   if (posN.first == H - 1 && posN.second == L - 1 && posB.first == 0 &&
       posB.second == 0)
      return true;
   for (auto i : Direction) {
      for (auto j : Direction) {
         if (explore(G, visited, posN + i, posB + j)) return true;
      }
   }
   return false;
}
