...
int main()
{
    ...
    while (!nextToMark.empty())
    {
        ...
        for (auto i : Direction)
        {
            ...
            if (sortDeLaGrille(nextP))
                continue;

            if (G[nextP.first][nextP.second] != -1)
                continue;

            G[nextP.first][nextP.second] = G[actuelle.first.first][actuelle.first.second] + 1;
