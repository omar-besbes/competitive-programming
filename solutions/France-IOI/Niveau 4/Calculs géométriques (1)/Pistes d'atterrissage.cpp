...
int prodVectoriel(const pair<int, int> &composantes_A,
                  const pair<int, int> &composantes_B) {
   return composantes_A.first * composantes_B.second -
          composantes_A.second * composantes_B.first;
}

int prodScalaire(const pair<int, int> &composantes_A,
                 const pair<int, int> &composantes_B) {
   return (composantes_A.first * composantes_B.first) +
          (composantes_A.second * composantes_B.second);
}
...
double distance_PtoL(pair<int, int> &P, pair<int, int> &A, pair<int, int> &B) {
   ...
   double distanceOrienteeAH =
       prodScalaire(composantes(A, B), composantes(A, P));
...
int main() {
   ...
   for (int i = 0; i < N; ++i) {
      ...
      if (D[i] < D[indexMini]) indexMini = i;
   }
   cout << piste[indexMini].first.first << ' ' << piste[indexMini].first.second
        << ' ';
   cout << piste[indexMini].second.first << ' '
        << piste[indexMini].second.second;
