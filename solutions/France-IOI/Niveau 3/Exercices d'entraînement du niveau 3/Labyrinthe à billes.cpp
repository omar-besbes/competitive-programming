...
void move(int dir, int B)
{
    ...
    if (nextL < 0 || nextC < 0 || nextL >= L || nextC >= C)
        return;
    ...
    else if (grid[nextL][nextC] == 'x')
    {
        ...
        for (unsigned int i = 0; i < ball.size(); i++)
        {
            if (i != B && ball[i].first == nextL && ball[i].second == nextC)
            {
                ...
                // the ball is somewhere in the same direction, so this ball must be behind it
                else
                {
                    // cout << "ball ahead at:" << ball[i].first << " " << ball[i].second << "\n";
