...
class SegmentTreeSolution : public Solution {
  private:
   ...
   long long query(int type, pair<int, int> range, int cur = 1, int l = 0,
                   int r = -1) {
      if (r == -1) r = n - 1;
      ...
      int m = (l + r) / 2;
      auto left = query(type, make_pair(range.first, min(range.second, m)),
                        cur * 2, l, m);
      auto right = query(type, make_pair(max(range.first, m + 1), range.second),
                         cur * 2 + 1, m + 1, r);
      return left + right;
   }
  ...
  private:
   ...
   long long query(int type, int l, int r) {
      ...
      if (first_blk == last_blk) {
         ...
         for (int i = l; i < (first_blk + 1) * blk_len; i++) ans += a[i];
         for (int i = first_blk + 1; i < last_blk; i++) ans += blk_a[i];
         for (int i = last_blk * blk_len; i <= r; i++) ans += a[i];
      }
