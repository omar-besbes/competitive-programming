...
long long query(pair<int, int> range, int v = 1, int l = 0, int r = t - 1)
{
    ...
    auto right = query(make_pair(max(m + 1, range.first), range.second), 2 * v + 1, m + 1, r);
    return left + right;
}
...
bool check_tree()
{
    ...
    {
        int i = first_appearance[get<1>(arr[k])], j = first_appearance[get<2>(arr[k])], d = get<0>(arr[k]);
        if (i > j)
            swap(i, j);
        if (query(make_pair(i, j - 1)) != d)
            return false;
    }
    return true;
}
