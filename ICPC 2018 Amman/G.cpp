...
bool compare(pair<int, pair<int, long long>> &a, pair<int, pair<int, long long>> &b) {
...
//    freopen("output.txt", "w+", stdout);
    ...
    while (t--) {
        ...
        for (auto &i:occ_score) {
            i.second.second = i.second.first * (i.second.first - 1) * (i.first) / 2;
        ...
        if(k >= n) {
            ...
            if(k < n-occ_score[25].second.first) {
                ...
                total += (occ_score[25].second.first + k) * (occ_score[25].second.first + k -1) * occ_score[25].first /2;
            } else {
